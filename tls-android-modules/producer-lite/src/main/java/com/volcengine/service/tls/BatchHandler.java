...
public class BatchHandler extends Thread {
    ...
    public void run() {
        ...
        for (BatchLog batchLog : batchLogList) { handle(batchLog); }
    }
    ...
    private void handle(BatchLog batch) {
        try { batch.fireCallbacks(); }
        catch (Throwable t) { LOG.error("batch fire callbacks failed: ", t); }
        finally { batchCount.decrementAndGet(); memoryLock.release(batch.getCurrentBatchSize()); }
    }

    @Override
    public void start() { this.closed = false; super.start(); }
