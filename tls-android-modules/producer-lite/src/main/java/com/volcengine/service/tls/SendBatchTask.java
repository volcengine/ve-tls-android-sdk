...
public class SendBatchTask implements Runnable {
    ...
    private void putBatchLogToFailureQueue() {
        try { failureQueue.put(batchLog); }
        catch (InterruptedException ex) { }
    }

    private boolean needRetry(LogException e) {
        return ProducerConfig.needRetry(e.getHttpCode()) && batchLog.getAttemptCount() <= producerConfig.getRetryCount() && !retryManager.isClosed();
    }
    ...
    private void handleLogException(LogException e) {
        ...
        if (needRetry(e)) {
            try { retryManager.put(batchLog); return; }
