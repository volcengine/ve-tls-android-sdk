...
public class ProducerConfig {
    ...
    public ProducerConfig(String endpoint, String region, String accessKey, String accessSecret) { this(endpoint, region, accessKey, accessSecret, null); }

    public static boolean needRetry(int httpCode) { return httpCode == TOO_MANY_REQUEST_ERROR || httpCode >= EXTERNAL_ERROR || httpCode == 0; }
    ...
    private long validNumber(Number field, Number min, Number max, Number originDefault) {
        ...
        if (fieldValue > maxValue || fieldValue < minValue) { return defaultValue; }
        return fieldValue;
    }

    public void setTotalSizeInBytes(int totalSizeInBytes) throws LogException {
        if (totalSizeInBytes <= 0) { throw new LogException("InvalidArgument", "totalSizeInBytes must be greater than zero,actual:" + totalSizeInBytes, null); }
        this.totalSizeInBytes = totalSizeInBytes;
    }
    public void setMaxThreadCount(int maxThreadCount) throws LogException {
        if (maxThreadCount <= 0 || maxThreadCount > MAX_THREAD_COUNT) { throw new LogException("InvalidArgument", String.format("maxThreadCount must between 1 to %d,actual:%d", MAX_THREAD_COUNT, maxThreadCount), null); }
        this.maxThreadCount = maxThreadCount;
    }
    public void setMaxBatchSizeBytes(int maxBatchSizeBytes) throws LogException {
        if (maxBatchSizeBytes <= 0 || maxBatchSizeBytes > MAX_BATCH_SIZE) { throw new LogException("InvalidArgument", String.format("maxBatchSizeBytes must between 1 to %d,actual:%d", MAX_BATCH_SIZE, maxBatchSizeBytes), null); }
        this.maxBatchSizeBytes = maxBatchSizeBytes;
    }
    public void setMaxBatchCount(int maxBatchCount) throws LogException {
        if (maxBatchCount <= 0 || maxBatchCount > MAX_BATCH_COUNT) { throw new LogException("InvalidArgument", String.format("maxBatchCount must between 1 to %d,actual:%d", MAX_BATCH_COUNT, maxBatchCount), null); }
        this.maxBatchCount = maxBatchCount;
    }
    public void setLingerMs(int lingerMs) throws LogException {
        if (lingerMs < MIN_WAIT_MS) { throw new LogException("InvalidArgument", String.format("maxWaitMs must be greater than %d,actual:%d", MIN_WAIT_MS, totalSizeInBytes), null); }
        this.lingerMs = lingerMs;
    }
    public void setRetryCount(int retryCount) throws LogException {
        if (retryCount <= 0 || retryCount > MAX_RETRY_COUNT) { throw new LogException("InvalidArgument", String.format("retryCount must between 1 to %d,actual:%d", MAX_RETRY_COUNT, retryCount), null); }
    ...
    public void setShardCount(int shardCount) throws LogException { if (shardCount <= 0) { throw new LogException("InvalidArgument", "shardCount must be greater than zero,actual:" + shardCount, null); } this.shardCount = shardCount; }
    public void setMaxBlockMs(long maxBlockMs) throws LogException { if (maxBlockMs <= 0) { throw new LogException("InvalidArgument", "maxBlockMs must be greater than zero,actual:" + maxBlockMs, null); } this.maxBlockMs = maxBlockMs; }
    public void checkBatchSize(int batchSize) throws LogException {
        if (batchSize > MAX_BATCH_SIZE) { throw new LogException("Invalid Arguments", "log batch size " + batchSize + " is larger than MAX_BATCH_SIZE " + MAX_BATCH_SIZE, null); }
        if (batchSize > getTotalSizeInBytes()) { throw new LogException("Invalid Arguments", "log batch size " + batchSize + " is larger than DEFAULT_TOTAL_SIZE_IN_BYTES " + DEFAULT_TOTAL_SIZE_IN_BYTES, null); }
    }

    public int getTotalSizeInBytes() { return totalSizeInBytes; }
    ...
    public void setGroupTags(java.util.Map<String,String> groupTags) { this.groupTags = groupTags; }
    public boolean isEnableTimeNs() { return enableTimeNs; }
    public void setEnableTimeNs(boolean enableTimeNs) { this.enableTimeNs = enableTimeNs; }
}
