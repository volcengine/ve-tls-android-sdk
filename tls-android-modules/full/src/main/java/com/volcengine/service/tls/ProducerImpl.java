...
public class ProducerImpl implements Producer {
    ...
    public void sendLogsV2(String hashKey, String topicId, String source, String filename, List<LogItem> logs, CallBack callBack)
        ...
        // 2 create batch log and add to dispatcher
        dispatcher.addBatch(hashKey, topicId, source, filename, AdaptorUtil.logItems2PbGroupWithTags(filename, source, producerConfig.getGroupTags(), logs, producerConfig.isEnableTimeNs()),
                callBack);
    }
    ...
    public void start() throws LogException {
        ...
        failHandler.start();
        mover.start();
        LOG.info(String.format("producer %s started, agent=%s, %s", name, SDKVersion.getAGENT("full"), safeConfigSummary(producerConfig)));
    }
