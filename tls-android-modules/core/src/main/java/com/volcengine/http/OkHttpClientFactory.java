...
public class OkHttpClientFactory {
    private static class ClientHolder {
        ...
        static {
            ...
            if (StringUtils.isNotBlank(hostname)) {
                ...
                if (StringUtils.isNotBlank(portP)) { port = Integer.parseInt(portP); }
    ...
    public static OkHttpClient create(ClientConfiguration conf, Proxy proxy, Interceptor... interceptors) {
        ...
        builder.proxy(proxy).connectTimeout(conf.getConnectionTimeout(), TimeUnit.MILLISECONDS).readTimeout(conf.getSocketTimeout(), TimeUnit.MILLISECONDS);
        for (Interceptor interceptor : interceptors) { builder.addInterceptor(interceptor); }
    ...
    public static OkHttpClient create() { return ClientHolder.INSTANCE; }
    public static OkHttpClient setConnectionTimeout(OkHttpClient.Builder builder, int connectionTimeout) { return builder.connectTimeout(connectionTimeout, TimeUnit.MILLISECONDS).build(); }
    public static OkHttpClient setSocketTimeout(OkHttpClient.Builder builder, int socketTimeout) { return builder.readTimeout(socketTimeout, TimeUnit.MILLISECONDS).build(); }
}
