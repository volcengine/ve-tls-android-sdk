...
import static com.volcengine.util.Const.LZ4;
import static com.volcengine.util.Const.ZLIB;

public class EncodeUtil {
    private static boolean LZ4_AVAILABLE;
    private static LZ4Factory factory;
    static {
        try {
            factory = LZ4Factory.fastestInstance();
            LZ4_AVAILABLE = (factory != null);
        } catch (Throwable t) {
            LZ4_AVAILABLE = false;
            factory = null;
        }
    ...
    public static byte[] lz4Decompress(byte[] data, int bodyRawSize) {
        ...
        if (!LZ4_AVAILABLE) {
            try { return zlibDecompress(data); } catch (DataFormatException e) { throw new RuntimeException(e); }
        }
