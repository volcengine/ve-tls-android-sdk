...
- Producer 配置参数说明：见 `SDK_USAGE_GUIDE.md` 中的 “Producer 配置参数说明（LogProducerConfig）”
...
phase-1 行为约定：
- 开启 persistent 前必须显式 `setPersistentFilePath(...)`；`Context` 构造器仅为兼容性保留，不会自动推导默认目录。
- `destroyLogProducer()` 是异步有界 best-effort；如果调用方需要观测本地 destroy 是否在给定预算内完成，可显式调用 `awaitDestroy(timeoutMs)`。
- `callbackFromSenderThread=false` 表示主线程回调契约；若主线程分发无法满足，必须显式告警/失败，不能静默回落到 sender thread。
