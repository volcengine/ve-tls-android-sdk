## 2.1.4
- 补全发布文档与导航，统一接入、持久化恢复、配置参考和运维指南。
- 保留模块的 Java 依赖升级至 Fastjson 1.2.84、Protobuf 3.25.8 和 at.yawk.lz4:lz4-java 1.10.1，修复已知依赖漏洞。
- 移除接入示例中未使用的 Java LZ4 依赖；Producer 主发布物不新增上述 Java 依赖。
- 保持 C Core v0.3.2、API 19 标准支持及 API 16 定制构建入口不变。

## 2.1.3
- 64 位 native 库增加 16 KB ELF 对齐，保留 API 19 主发布基线；补齐宿主打包兼容说明。
