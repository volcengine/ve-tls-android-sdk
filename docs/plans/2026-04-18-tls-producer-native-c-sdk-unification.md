...
Every wave is a formal execution phase. Do not start a phase until its dependencies are satisfied and its predecessor is marked `Done` in the tracker.

| Phase | Goal | Depends on | Directories / modules | Verification commands | Done when | Must stop immediately if |
| --- | --- | --- | --- | --- | --- | --- |
| Wave 0 | establish live execution baseline before coding | none | workspace root, `ve-tls-c-sdk`, `ve-tls-android-sdk`, tracker docs | `git -C ve-tls-c-sdk status --short`; `git -C ve-tls-android-sdk status --short`; `git -C ve-tls-c-sdk log --oneline -5`; `git -C ve-tls-android-sdk log --oneline -5` | tracker updated with current wave, repo status, latest commits, next runnable tasks | either repo has unexpected dirty changes that conflict with the active task; tracker and working tree disagree; spec/plan/tracker point to different next steps |
| Wave 1 | create the minimal native and Android scaffolding to unblock later integration | Wave 0 | `ve-tls-c-sdk/bindings/android`, `ve-tls-c-sdk/tests`, `ve-tls-c-sdk/CMakeLists.txt`, `ve-tls-android-sdk/tls-android-modules/producer-native`, `ve-tls-android-sdk/tls-android-modules/integration-tests`, publish metadata files | `cmake -S ve-tls-c-sdk -B /tmp/ve_tls_android_plan_build -DVE_TLS_BUILD_TESTS=ON -DVE_TLS_BUILD_TOOLS=OFF`; `cmake --build /tmp/ve_tls_android_plan_build --target ve_tls_test_android_binding`; `cd ve-tls-android-sdk/tls-android-modules && ./gradlew :producer-native:assembleRelease` | Task 1 and Task 3 are both review-green and committed; `producer-native` module exists and C SDK binding target compiles | CMake integration requires invasive core refactor; `producer-native` cannot be assembled without changing unrelated modules; the two parallel tasks start touching the same files |
| Wave 2 | solidify config/API boundaries on both repos while write sets are still disjoint | Wave 1 | `ve-tls-c-sdk/bindings/android`, `ve-tls-c-sdk/tests`, `ve-tls-android-sdk/tls-android-modules/producer-native/src/main/java`, Android unit tests | `cmake --build /tmp/ve_tls_android_plan_build --target ve_tls_test_android_binding`; `ctest --test-dir /tmp/ve_tls_android_plan_build --output-on-failure -R ve_tls_test_android_binding`; `cd ve-tls-android-sdk/tls-android-modules && ./gradlew :producer-native:testDebugUnitTest --tests \"*LogProducerConfigTest\" --tests \"*LogProducerResultTest\"` | Task 2 and Task 4 are both review-green and committed; config mirror, path rewrite helpers, and public Java API boundaries are stable | config model requires spec change; public Java API write set begins to overlap pending JNI/HTTP work; either task fails spec review with architectural mismatch |
| Wave 3 | implement Java-side lifecycle orchestration without JNI coupling | Wave 2 | `ve-tls-android-sdk/tls-android-modules/producer-native/src/main/java/com/volcengine/tls/android/producer`, `.../internal`, Java unit tests | `cd ve-tls-android-sdk/tls-android-modules && ./gradlew :producer-native:testDebugUnitTest --tests \"*LogProducerClientBridgeTest\"` | Task 5 is review-green and committed; config snapshot, process path rewrite, and destroy-state guarding are stable | lifecycle logic cannot be expressed without immediately changing JNI signatures; process/path semantics contradict the approved spec |
| Wave 4 | connect Java lifecycle to real native producer creation/update/destroy | Wave 3 and Wave 2 C SDK output | `ve-tls-android-sdk/tls-android-modules/producer-native/src/main/cpp`, `.../internal/JniNativeProducerBridge.java`, `ve-tls-c-sdk/bindings/android` | `cd ve-tls-android-sdk/tls-android-modules && ./gradlew :producer-native:testDebugUnitTest --tests \"*NativeApiContractTest\"`; `./gradlew :producer-native:assembleRelease` | Task 6 is review-green and committed; JNI can create, update, reset credentials, and destroy with bounded close ordering | JNI lifecycle bridge requires spec changes; native create/destroy is unstable or leaks handles; Android build cannot link `ve_tls_core` cleanly |
| Wave 5 | implement the production HTTP bridge and Android TLS transport mapping | Wave 4 | `ve-tls-android-sdk/tls-android-modules/producer-native/src/main/java/.../NativeHttpBridge.java`, `.../src/main/cpp/tls_producer_jni.cpp`, `ve-tls-c-sdk/bindings/android` | `cd ve-tls-android-sdk/tls-android-modules && ./gradlew :producer-native:testDebugUnitTest --tests \"*NativeHttpBridgeTest\"`; `./gradlew :producer-native:assembleRelease` | Task 7 is review-green and committed; HTTP timeouts, TLS verification, and JNI thread attachment rules are implemented and verified | TLS verification or CA loading semantics cannot be implemented safely; native sender threads crash or deadlock during JNI attachment; HTTP bridge requires third-party networking contrary to spec |
| Wave 6 | wire end-to-end add-log flow, callback mapping, demo, and integration smoke tests | Wave 5 | `ve-tls-android-sdk/tls-android-modules/producer-native/src/main/java`, `.../src/main/cpp`, `tls-android-modules/app`, `tls-android-modules/integration-tests` | `cd ve-tls-android-sdk/tls-android-modules && ./gradlew :producer-native:testDebugUnitTest`; `./gradlew :integration-tests:test --tests \"com.volcengine.integration.ProducerNativeApiContractTest\"`; `./gradlew :app:assembleDebug` | Task 8 is review-green and committed; add-log JNI path, callback mapping, and demo app compile successfully | callback mapping diverges from spec; add-log path requires reintroducing old Java producer internals; integration tests expose unresolved lifecycle or transport regressions |
| Wave 7 | remove legacy producer code and complete docs/publishing cutover | Wave 6 | `ve-tls-android-sdk/tls-android-modules/producer-lite`, `.../full`, `.../settings.gradle`, `.../app/build.gradle`, `README.md`, `SDK_USAGE_GUIDE.md`, `RELEASE.md`, `CHANGELOG.md` | `cmake -S ve-tls-c-sdk -B /tmp/ve_tls_android_plan_build -DVE_TLS_BUILD_TESTS=ON -DVE_TLS_BUILD_TOOLS=OFF`; `cmake --build /tmp/ve_tls_android_plan_build --target ve_tls_test_android_binding`; `ctest --test-dir /tmp/ve_tls_android_plan_build --output-on-failure`; `cd ve-tls-android-sdk/tls-android-modules && ./gradlew :producer-native:assembleRelease :core:assembleRelease :full:assembleRelease :integration-tests:test` | Task 9 is review-green and committed; `producer-native` is the only formal producer path and docs/publishing match reality | any required verification is red; cleanup would delete code still needed by `full` synchronous client; migration docs are inconsistent with the shipped API |

## Global Stop Conditions

Regardless of phase, stop and update the tracker immediately if any of these happens:

- a task requires changing the approved spec rather than implementing it
- two running tasks need to edit the same file or same tightly coupled boundary
- either repo has unexpected user changes in the active write set
- verification output contradicts the tracker's claimed status
- a subagent reports `BLOCKED` twice on the same task without new context
- a task passes tests but fails spec review or code review
